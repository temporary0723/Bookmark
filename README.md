...
SillyTavern용 고급 북마크 확장으로, 채팅 메시지에 북마크를 추가하고 효율적으로 관리할 수 있는 강력한 기능을 제공합니다.

## ✨ 주요 기능

### 📌 북마크 추가
- 각 메시지에 북마크 아이콘이 자동으로 추가됩니다
- 아이콘을 클릭하면 북마크 이름을 입력할 수 있는 모달이 나타납니다
- 북마크된 메시지는 시각적으로 구분되어 표시됩니다

### 📚 북마크 관리
- 요술봉 메뉴에서 "북마크 관리" 버튼을 통해 북마크 목록에 접근
- 저장된 모든 북마크를 메시지 ID 순으로 정렬하여 표시
...
- 휴지통 아이콘으로 북마크 삭제 (확인 다이얼로그 제공)

### 🎯 메시지 이동
- 북마크 항목 클릭 시 해당 메시지로 자동 스크롤
- 이동한 메시지는 하이라이트 효과로 3초간 강조
- SillyTavern 공식 `/chat-jump` 명령어 사용

### 💾 데이터 관리
- **데이터 내보내기**: 모든 캐릭터의 북마크를 JSON 파일로 내보내기
- **데이터 불러오기**: v1.0, v2.0, v3.0 형식 파일 지원
- **전체 삭제**: 모든 캐릭터의 북마크를 안전하게 일괄 삭제

### ⚡ 성능 최적화
- **인덱스 기반 시스템**: Extension Settings에 북마크 인덱스 저장
- **빠른 전체 내보내기**: 북마크가 있는 캐릭터만 선별 처리
...
### 데이터 관리
1. 북마크 목록 모달에서 다음 버튼들을 사용할 수 있습니다:
   - **📥 데이터 불러오기**: JSON 파일에서 북마크 가져오기
   - **📤 데이터 내보내기**: 모든 캐릭터의 북마크를 JSON 파일로 저장
   - **🗑️ 전체 삭제**: 모든 캐릭터의 북마크를 안전하게 삭제

## 💾 데이터 저장 방식

### 하이브리드 저장 시스템
- **북마크 데이터**: 각 캐릭터의 채팅 메타데이터에 저장
- **북마크 인덱스**: SillyTavern Extension Settings에 저장
- **자동 동기화**: 채팅 변경 시 자동으로 북마크 로드
...
### v2.1.0 (최신)
- 🚀 **인덱스 기반 성능 최적화**: Extension Settings에 북마크 인덱스 저장
- ⚡ **빠른 전체 내보내기**: 북마크가 있는 캐릭터만 선별 처리 (80%+ 성능 향상)
- 🔄 **실시간 모달 업데이트**: 데이터 변경 시 즉시 UI 반영
- 💾 **데이터 관리 기능**: 불러오기/내보내기/전체 삭제
- 🗑️ **안전한 전체 삭제**: 2단계 확인으로 실수 방지
- 📁 **v3.0 형식 지원**: 모든 캐릭터 북마크 포함
...
- **인덱스 최적화**: ~8초 (북마크 보유 캐릭터만 처리)
- **성능 향상**: **80%+ 단축** ⚡

### 메모리 사용량
- **북마크 인덱스**: ~5KB (1000개 북마크 기준)
...
**💡 팁**: 많은 북마크를 관리하신다면 정기적으로 데이터를 내보내서 백업하는 것을 권장합니다!